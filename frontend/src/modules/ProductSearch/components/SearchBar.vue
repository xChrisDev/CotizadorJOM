...
<template>
  <div class="relative w-full max-w-xl" ref="inputRef">
    <input v-model="searchQuery" placeholder="Buscar..."
      class="pl-12 pr-10 w-full py-2 border-2 border-[#4ed636] rounded-lg focus:outline-none"
    ...
    <teleport to="body">
      <ul v-if="showDropdown"
        class="search-dropdown absolute mt-2 z-[99999] bg-white dark:bg-[#27272a] border-2 border-[#4ed636] rounded-lg shadow-xl max-h-70 overflow-y-auto scrollbar-transparent p-2"
        :style="{
          ...
          <li v-for="(item, index) in results" :key="index" @click="selectItem(item)"
            ...
            <div class="flex flex-col overflow-hidden">
              ...
              <span class="text-xs text-gray-500 truncate">
                {{ item.category.name }} | {{ item.family.name }}
              </span>
            </div>
          </li>
