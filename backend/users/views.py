...
def login(request):
    ...
    if not user.check_password(request.data.get("password")):
        return Response(
            {"error": "Contraseña no valida"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if serializer.data.get("status") == "PENDING":
        return Response(
            {"error": "Aun no se ha aceptado tu solicitud."},
    ...
    data = {"username": serializer.data["username"], "rol": serializer.data["rol"]}
...
def register(request):
    ...
    if serializer.is_valid():
        ...
        return Response(
            {"success": serializer.data["username"]}, status=status.HTTP_201_CREATED
        )
